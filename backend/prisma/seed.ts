...
//   console.log(`👤 Created mock user account: ${mockUser.email}`);
...
async function main() {
  ...
  const mockOrganizer = await prisma.user.upsert({
    where: {
      email: 'organizer@test.com',
    },
    update: {},
  ...
  console.log(`👤 Mock organizer account ready: ${mockOrganizer.email}`);
  ...
  console.log(`👤 Mock user account ready: ${mockUser.email}`);
  ...
  // Only seed demo events if none exist
    ...
    for (const item of initialEvents) {
      ...
      await prisma.event.create({
        data: {
          ...
          capacity: item.capacity,
          availableSeats:
            item.capacity - Math.floor(Math.random() * 5),
          organizerId: mockOrganizer.id,
        },
      });
    }
